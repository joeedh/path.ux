...
port; most fixes are mechanical _applications_ of these patterns.
...
```ts
SomeClass.STRUCT =
  nstructjs.inherit(SomeClass, Base, "namespace.Name") +
  `
  field : type;
}`;
...
**TS2417 fix (confirmed):** the error is _"types returned by define()/blockDefine() are
incompatible"_ — subclass return literal is missing properties the base returns. Two clean
fixes:
...
- `class Context<Overlays extends ContextLike = ContextLike>` with **`state: unknown`** and
  **`constructor(appstate: unknown)`** (so `super(appstate)` in `ToolContext`/`ViewContext`
  is fine — the TS2554 "expected 0, got 1" comes from a _different_ call; find it in
...
arrays type-checks _as long as the global augmentation is in scope_. It is reachable through
