...
export class IconButton<CTX extends IContextBase = IContextBase, VALUE = unknown> extends UIBase<CTX, VALUE> {
  ...
  setCSS() {
    ...
    const tileSize = ui_base.iconmanager.getTileSize(this.iconsheet);
    const size = this.getDefault("iconSize", true, undefined) ?? tileSize;
    const w = size;
  ...
  getIconSize() {
    const tileSize = ui_base.iconmanager.getTileSize(this.iconsheet);
    const size = this.getDefault<number>("iconSize", true, undefined) ?? tileSize;
    return size;
  }
  ...
  _getsize() {
    const margin = this.getDefault<number>("padding");
    const tileSize = ui_base.iconmanager.getTileSize(this.iconsheet);
    const size = this.getDefault<number>("iconSize", true, undefined) ?? tileSize;
