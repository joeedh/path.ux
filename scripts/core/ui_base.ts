...
> extends HTMLElement {
  ...
  _clipboardHotkeyInit(): void {
    ...
    this._clipboard_keydown = (e: KeyboardEvent, internal_mode?: boolean) => {
      ...
      if (!internal_mode) {
        ...
        checkTree = checkTree || (is_copy && (this.constructor as unknown as typeof UIBase).define().copyForAllChildren)
  ...
  */
  ...
  getStyleClass(ignoreOverride = false): string {
    if (!ignoreOverride && this._override_class !== undefined) {
  ...
  private getStyleRecord(styleClass: string, key?: string, inherit = true): ThemeRecord | undefined {
    ...
    let p: UIBase | undefined = this
    while (p) {
      let def1 = p.class_default_overrides[chunk1]
      ...
      let def2 = p.class_default_overrides[styleClass]
  ...
  getClassDefault(key: string, checkForMobile = true, defaultval?: unknown, inherit = true): unknown {
    ...
    let record = this.getStyleRecord(style, key, inherit) as any
    //return record ? record[key] ?? defaultval : defaultval
    ...
    if (value === undefined && defaultval !== undefined) {
      ...
      if (this._override_class !== undefined) {
        const record2 = this.getStyleRecord(this.getStyleClass(true), key, inherit)
