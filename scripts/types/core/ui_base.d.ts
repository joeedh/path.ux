...
declare class UIBase<CTX extends Context = Context> extends HTMLElement {
  ...
  *  (e.g. you can't modifer .style or set attributes)*/
  ...
  getPathMeta<T = any, PropType extends ToolProperty<any> = ToolProperty<any>>(ctx: any, path: string): PathPropMeta<T, P> | undefined

  //getPathMeta<T = any>(ctx: any, path: string)
  loadNumConstraints(prop: ToolProperty, dom: HTMLElement, onModifiedCallback = (elem: UIBase) => any);
  ...
  getDefault(key: string, checkForMobile?: boolean, defaultval?: string | number, inherit?: boolean): string | number | boolean;

  getStyleClass(): string;

  hasClassDefault(key: string): boolean;

  getClassDefault(key: string, checkForMobile?: boolean, defaultval?: string | number, inherit?: boolean): string | number | boolean;
